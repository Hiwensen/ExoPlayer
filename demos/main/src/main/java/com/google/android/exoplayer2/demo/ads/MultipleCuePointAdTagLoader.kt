...
class MultipleCuePointAdTagLoader(private val adTagDataSpec: DataSpec, private val adsId: Any) :
    ...
    override fun onReceiveAds(
        ...
        val adGroupTimeMinute = TimeHelper.microToMinute(adPlaybackState.getAdGroup(adGroupIndex).timeUs)
        Log.d(TAG, "onReceiveAds, " +
                "adGroupIndex:$adGroupIndex, timeMin:${adGroupTimeMinute}")
...
//                adPlaybackState = adPlaybackState.withAdDurationsUs(adGroupIndex, *adGroupDurationUs.toLongArray())
    ...
    override fun onPositionDiscontinuity(
        ...
        if (reason == Player.DISCONTINUITY_REASON_SEEK) {
            onSeek(Util.msToUs(oldPosition.positionMs), Util.msToUs(newPosition.positionMs))
        }
        handleTimelineOrPositionChanged()
    }
    ...
    private fun onSeek(oldPositionUs: Long, newPositionUs: Long) {
        ...
        for(adGroupIndex in 0 until adPlaybackState.adGroupCount) {
            ...
            if (adGroup.timeUs in startPosition..endPosition) {
                if (oldPositionUs < newPositionUs) {
                    // seek forward
                    ...
                    // reset ad group
                    Log.d(TAG,"handle seek rewind, resetAdGroup index:$adGroupIndex}")
                    adPlaybackState = adPlaybackState.withResetAdGroup(adGroupIndex)
                    val originalAdGroup = originalAdPlaybackState.getAdGroup(adGroupIndex)
                    if (originalAdGroup.timeUs != adGroup.timeUs) {
                        Log.d(
                                TAG,
                                "handle seek rewind, revert adGroupTime:$adGroupIndex, " +
                                        "adGroup min:${TimeHelper.microToMinute(adGroup.timeUs)}," +
        ...
        if (C.INDEX_UNSET != latestSkippedAdGroupIndex &&
            ...
            // whenever seek and skip one cue point, update the latest cue point before the new position
            val originPositionUs = originalAdPlaybackState.getAdGroup(latestSkippedAdGroupIndex).timeUs
            Log.d(
                TAG,
                "handle seek forward, update cuePoint position, adGroupIndex:$latestSkippedAdGroupIndex, " +
                    "newPositionMin:${TimeHelper.microToMinute(newPositionUs)}," +
                        "oldPositionMin:${TimeHelper.microToMinute(originPositionUs)}"
            )
            Log.d(
                "adsLoaderDebug_player",
                "handle seek forward, update cuePoint position, adGroupIndex:$latestSkippedAdGroupIndex, " +
